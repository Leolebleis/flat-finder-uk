...
```
GET https://api.tfl.gov.uk/Journey/JourneyResults/{lat},{lng}/to/51.5074,-0.1278
    ?mode=tube,bus,overground,elizabeth-line,dlr,tram
    &time=0830&timeIs=arriving
```

Destination: Example Destination (51.5074, -0.1278). Hardcoded for now.
...
- `commute_mins INTEGER` -- public transport minutes to the destination
...
- **Commute destination hardcoded**: the destination. Making it configurable is easy later but YAGNI.
