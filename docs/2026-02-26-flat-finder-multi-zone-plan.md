...
**Files:**
- Create: `config/flat-finder/zones.json` (on Pi at `/opt/services/config/flat-finder/zones.json`)
- Modify: Docker compose in `/opt/services/docker-compose.yml`
- Modify: `scraper/Dockerfile`

**Step 1: Create zones.json on Pi**

```bash
sudo mkdir -p /opt/services/config/flat-finder
```

Write `/opt/services/config/flat-finder/zones.json` with the two zones (Finchley Road + St John's Wood).
...
- Check VPS API: `curl -s -H "X-API-Key: <key>" https://example.com/flat/api/stats`
