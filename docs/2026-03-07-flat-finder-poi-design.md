...
| name | TEXT NOT NULL | Display name, e.g. "Rue's", "Gym" |
...
2. If `pois` is empty and `commute_mins` column exists in listings: seed two POIs (Work at 51.4869/-0.1832, Gym at 51.5445/-0.1762) with current hardcoded coordinates
...
Include commute times for all POIs in notification text: "35min to Rue's, 10min to Gym".
